	...
	function output ($entry, $way='excerpt', $contentonly=false) {
		...
		if ($flset['tags']!=1 && $entry['tags'] && $entry['tags']!='>') {
			...
			foreach ($taginfo as $eachtag) {
				...
				$taginfos[]="<span class=\"single-tag\">#<a href=\"{$urlref}\" title=\"Tags:  {$eachtag}\" rel=\"tag\">{$eachtag}</a></span>";
			}
		...
		if ($permission['EditEntry']!=1) $ifadmin='';
			...
			$ifadmin=" &#8226; {$entry['views']} Reads &#8226; <a href=\"javascript: showdelblog('{$entry['blogid']}');\">{$lnc[43]}</a> &#8226; <a href=\"admin.php?go=edit_edit_{$entry['blogid']}\">{$lnc[42]}</a>" ;
		...
		if ($entry['comefrom'] && $entry['originsrc']) {
			$entrysourcewithlink="<a href=\"{$entry['originsrc']}\" >{$entry['comefrom']}</a>";
		}
		else {
