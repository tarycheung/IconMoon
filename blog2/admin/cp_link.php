...
if ($job=='detail') {
	...
	for ($i=0; $i<count($detail_array); $i++) {
		...
		$tablebody.="<tr class='$addclass'><td align='center'><input type='checkbox' name='selid[]' id='selid[]' value='{$detail_array[$i]['linkid']}'></td><td align='center'>{$imgsign}</td><td>{$detail_array[$i]['linkname']}</td><td><a href='{$detail_array[$i]['linkurl']}'>{$detail_array[$i]['linkurl']}</a></td><td align='center'>{$linkgps[$tmp_gp]['linkgpname']}</td><td align='center'>{$hiddensign}</td><td align='center'><a href='admin.php?go=link_modify_{$detail_array[$i]['linkid']}'><img src='admin/theme/{$themename}/edit.gif' alt='{$lna[77]}' title='{$lna[77]}' border='0'></a></td></tr>";
	}
	...
	if (!file_exists($filename)) $tablebody="<tr><td colspan=10 align=center><br><br>{$lna[254]}<br><br></td></tr>";
		...
		if (trim($wlinks[0])=='') $tablebody="<tr><td colspan=10 align=center><br><br>{$lna[254]}<br><br></td></tr>";
			...
			foreach ($wlinks as $link) {
				...
				$sitelogo=($sitelogo) ? "<a href='$sitelogo' title='{$lna[255]}'><img src='admin/theme/{$themename}/view.gif' border='0'></a>" : "&nbsp;";
