...
<ul id="nav">
	...
	<li {if $pagename eq "published" || $pagename eq "index"}class="current"{/if}><a href='{$my_base_url}{$URL_base}'><i class="font-icon icon-star-alt"></i> {#PLIGG_Visual_Published_News#}</a></li>
	<li {if $pagename eq "upcoming"}class="current"{/if}><a href="{$URL_upcoming}"><i class="font-icon icon-idea-alt"></i> {#PLIGG_Visual_Pligg_Queued#}</a></li>
	<li {if $pagename eq "submit"}class="current"{/if}><a href="{$URL_submit}"><i class="font-icon icon-pencil-alt"></i> {#PLIGG_Visual_Submit_A_New_Story#}</a></li>
	...
	{/if}	
	{if $user_authenticated eq true}<li {if $pagename eq "user"}class="current"{/if}><a href="{$URL_userNoVar}"><span><i class="font-icon icon-group-alt"></i> {#PLIGG_Visual_Profile#}</span></a></li>{/if}
	{if isset($isgod) && $isgod eq 1}<li><a href="{$URL_admin}"><span><i class="font-icon icon-wrench-alt"></i> {#PLIGG_Visual_Header_AdminPanel#}</span></a></li>{/if}
